...
import {
  scoreTrust,
  scoreGrowth,
  summarizeTrustInsights,
  summarizeGrowthInsights,
  ...
  // generating runId at start
    ...
    try {
      ...
      if (!ticker || !pdfFiles?.length) {
        ...
        res.status(400).json({ error: 'ticker + at least 1 PDF file required' });
        return;
      }
    ...
    // 1. extract PDF texts
      ...
      for (let i = 0; i < pdfFiles.length; i++) {
        ...
        try {
          ...
          // we continue or abort; here we abort:
          res.status(500).json({ error: `Error extracting PDF file: ${msg}`, runId });
          return;
        }
      }
    ...
    // 2. urls per ticker (company)
      ...
      if (!urls) {
        ...
        res.status(400).json({ error: 'Unsupported ticker', runId });
        return;
      }
    ...
    // 5. summarize trust insights
      ...
      const trustSummaryJsonString = await summarizeTrustInsights(
        trustResult.score,
        trustResult.justifications,
        runId
      );
      ...
      try {
        ...
        trustSummaryObj = {
          overallJustification: trustSummaryJsonString,
          recommendations: [],
        };
      }
    ...
    // 7. summarize growth insights
      ...
      const growthSummaryJsonString = await summarizeGrowthInsights(
        growthResult.score,
        growthResult.justifications,
        runId
      );
      ...
      try {
        ...
        growthSummaryObj = {
          overallJustification: growthSummaryJsonString,
          recommendations: [],
        };
      }
